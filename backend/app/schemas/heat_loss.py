...
class TankHeatLossParams(BaseModel):
    ...
    q_additional: TankAdditionalHeatLoss = Field(
        ...
        description=(
            ...
            "(например, штуцера, опоры и фланцы). Днище уже учтено в площади "
            "резервуара; значение прибавляется после коэффициента запаса и увеличивает "
            "проектные удельные теплопотери."
        ),
    )
