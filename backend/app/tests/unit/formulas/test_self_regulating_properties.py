...
class TestGoldenElectrical:
    def test_example_from_formules_md(self):
        """Проверка электрических метрик на независимом входе req=60 Вт/м.

        Старое значение req=123 Вт/м было историческим числом из исправленного
        примера теплопотерь и не является источником этого golden-теста.
        L=50 м → L_кабеля=55 м
        P=P_каб × 55
        I=P/220

        Проверяем вычисления вокруг длины и тока с температурно допустимой маркой.
        """
