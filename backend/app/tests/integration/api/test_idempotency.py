...
async def _prepare_assigned_tank(
    client: AsyncClient,
    headers: dict[str, str],
    *,
    project_name: str,
    ...
    obj_response = await client.post(
        f"/api/v1/projects/{proj['id']}/objects",
        json={"object_type": "tank", "sort_order": 0, "params": TANK_PARAMS},
        headers=headers,
    )
...
class TestBatchCalcIdempotency:
    ...
    async def test_tank_batch_calc_succeeds_three_times_without_result_feedback(
        self,
        client: AsyncClient,
        employee_token: str,
    ):
        headers = {"Authorization": f"Bearer {employee_token}"}
        prepared = await _prepare_assigned_tank(
            client,
            headers,
            project_name="Idem-Tank",
        )
        proj = prepared["project"]
        variant = prepared["variant"]
        ...
        assert [response["calculated"] for response in responses] == [1, 1, 1]
        ...
        assert "winding_pitch" not in listing[0]["results"]
        assert "winding_pitch_mm" not in listing[0]["results"]["layout"]
