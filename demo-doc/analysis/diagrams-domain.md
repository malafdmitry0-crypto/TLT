...
- Результаты теплотехнического расчёта хранятся **внутри** `project_objects.results` (JSONB). Отдельной таблицы `calculations` нет — это сознательный трейдофф: на MVP достаточно последнего снимка результата.
...
- Результат теплотехнического расчёта в MVP хранится как JSONB в `ProjectObject.results` (не отдельная таблица). Диаграмма классов ниже показывает концептуальную модель; `CalculationResult` — это структура внутри JSONB, не SQL-таблица.
