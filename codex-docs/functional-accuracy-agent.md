...
## Универсальные усилители evidence

Эти проверки добавляются к минимальной матрице, когда функционал влияет на
расчетную точность, деньги, спецификацию, отчет, импорт/экспорт, права или
массовые операции:

| Риск | Что дополнительно доказать |
|---|---|
| Расчет и формула | Независимый golden/oracle, boundary cases, metamorphic checks; для критичного ядра - mutation gate |
| Версия результата | `formula_id`, версия формулы/коэффициентов/каталога, категория результата и `error_code` сохраняются и читаются обратно |
| API/UI контракт | Frontend отправляет те же единицы, обязательные параметры и дефолты, которые ожидает backend |
| Отчет/спецификация | Успешные, ошибочные, неподдержанные и устаревшие результаты разделены; бизнес-суммы считаются по выбранному варианту |
| Импорт/export/batch | Повторный запуск, дедупликация, стабильные ключи связей, лимиты и частичный успех имеют тест или явно описанный residual risk |
