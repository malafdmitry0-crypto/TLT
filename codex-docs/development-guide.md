...
- Публичный scope ЭР должен проходить сквозно как `electrical_variant_id`:
  расчёт, specification, report, URL/cache и UI. `variant_number=1…5` допустим
  только внутри явно обозначенного compatibility adapter и всегда сверяется с
  exact UUID.
...
- Редактируемая документация живёт в `docs/`. Демо-поставка генерируется по
  `docs/playbooks/demo-release.md` в ignored `demo/` и не является параллельным
