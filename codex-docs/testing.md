...
Для UI/layout/UX задач зелёный unit-test не является достаточным доказательством.
Минимальный gate:

1. Снять before screenshot или открыть существующий артефакт, где виден дефект.
2. Найти DOM/CSS/root cause: конкретный selector, компонент или стиль.
3. Исправить без изменения бизнес-контракта.
4. Добавить или усилить verifier, который ловит этот класс дефекта: clipping,
   `text-overflow`, overlap, horizontal scroll, disabled-but-needed controls,
   нечитаемый/обрезанный текст.
5. Снять after screenshot на целевом viewport. Для рабочего стола HeatCalc
   проверять минимум 1280/1366 px, а при затрагивании responsive layout также
   tablet/mobile viewport из `layout` gate.
6. Если Playwright/Chrome не запускается, задача получает статус `blocked` или
   `needs verification`. Нельзя закрывать UI-defect как fixed только по коду.

## Golden / Expected Policy

- Изменение golden number или ожидаемой мощности/теплопотерь допускается только
  при изменении источника истины: формулы, коэффициента, справочника, каталога
  или явно подтверждённого бизнес-правила.
- В diff должно быть видно, почему число изменилось: комментарий в тесте,
  ссылка на документ/справочник или отдельный расчётный oracle.
- Массовая замена fixture expected values без объяснения считается ослаблением
  тестового контура.
- Если тест был слишком строгим из-за старой ложной модели, новый диапазон
  должен описывать физический/бизнес-инвариант, а не просто покрывать текущий
  вывод кода.
