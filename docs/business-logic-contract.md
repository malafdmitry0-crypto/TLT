...
| ID | Правило | Источник | Реализация | Evidence |
|---|---|---|---|---|
| `tlt_spec_full_bom_rules` | Full BOM `mode=full` (PDF 07.07.2026 + PDL): **кабель** = commercial `required_order_length` (PDL-ER-02), не raw `Lсек×N`; **комплекты** pick-one `ceil(Nсек/sections_per_kit)` (PDL-ER-44); **repair** `ceil(ΣL_installed/cable_length_per_kit)` default 150; **клей** `ceil((conn+repair)/kits_per_unit)` default 7; **ленты** PDF formulas + `ceil(m/reel_m)`; **коробки** data-driven rows PDF-BOM-07 (`box_ex_rgr_matrix.json`); Rгр scales N for kits only, not cable order; dтр≥57 inclusive (PDL-ER-08); tank/resistive → proven cable only (PDL-ER-32) | `docs/tnp/cases/guest-specification/pdf-requirements.md`, `product-decisions.md` | `backend/app/formulas/specification/full_builder.py`, `spec_accessories.json`, `box_ex_rgr_matrix.json` | `backend/app/tests/unit/formulas/test_spec_full_builder.py` (`TestPdfBomGoldens`, boxes, kits) |
| `tlt_spec_bom_package_factor` | Упаковка: prefer `kits_per_unit` / `reel_m` / `cable_length_per_kit` from catalog; legacy `package_factor` fallback. Glass reel 30 m, Al reel 50 m, glue kits_per_unit 7 | PDF §7.11–7.14 + catalog | `spec_accessories.json`, `full_builder._package_quantity` | `TestPdfBomGoldens`, `test_package_factor_converts_to_packages` |
| `tlt_spec_full_bom_scope` | Full BOM: proven cable self-reg+resistive; accessories self-reg **pipe** only. Guest **full** allowed (PDL-ER-04/29). `bom_section` pipe/tank on items (PDL-ER-38). Multi-ER explicit UUID generate + preflight (PDL-ER-01/36). Settings snapshot + stale (PDL-ER-07) | PDF-SPEC + PDL-ER | `full_builder.py`, `specification_service.py`, `api/v1/specifications.py` | `test_spec_full_builder.py`, `test_specifications.py` |

Открытые вопросы полного BOM: **официальные** SEEDS (Lmax/Iдоп/Ex/Rгр factory tables)
вместо provisional dev-1.x; corporate report template (PDL-ER-40). Закрыто: dтр=57
inclusive, pick-one kits, dual-length order/installed, tank accessories partial
(PDL-ER-32). Исторические заметки: `docs/analysis/spec-bom-open-issues-2026-06-09.md`.
