...
|---|---|---|---|---|
| `tlt_pipe_total_heat_loss` | `Q = q_linear * (L + ΣLдоп) * K * K_разм`; `K_разм=location_indoor/location_outdoor` из `correction_coefficients` | `docs/tnp/block-heat-loss-and-cable-selection/pipe-heat-loss-and-cable-selection.md`; app policy для `K_разм` | `backend/app/formulas/heat_loss/pipe.py` | `backend/app/tests/unit/formulas/test_pipe_heat_loss.py`, `backend/app/tests/unit/formulas/test_pipe_properties.py` |
...
| `tlt_tank_total_heat_loss` | Для резервуара `Q = q * S * K * K_разм + Qдоп`; для подземного резервуара `Q=(q_возд*S_возд + q_гр*S_гр)*K*K_разм + Qдоп`; `Qдоп` прибавляется после множителей | `docs/tnp/block-heat-loss-and-cable-selection/tank-heat-loss-and-cable-selection.md`; app policy для `K_разм` | `backend/app/formulas/heat_loss/tank.py` | `backend/app/tests/unit/formulas/test_tank_heat_loss.py`, `backend/app/tests/unit/formulas/test_tank_properties.py` |
...
| `tlt_heat_loss_location_factor_source_gap` | `location_indoor=0.9` и `location_outdoor=1.0` применяются к итоговому `Q`, но в проверенных первичных ТНП DOCX/XLSX отдельный множитель размещения не найден. Это текущая app/product policy, требующая бизнес-подтверждения или отдельного первоисточника. | `docs/analysis/heat-loss-tz-deviations.md`; current app policy; source confirmation pending | `backend/app/formulas/heat_loss/common.py`, `backend/app/formulas/heat_loss/pipe.py`, `backend/app/formulas/heat_loss/tank.py` | `backend/app/tests/unit/formulas/test_pipe_heat_loss.py`, `backend/app/tests/unit/formulas/test_tank_heat_loss.py` |
...
| `tlt_heat_loss_location_factor_source_gap` | Needs business decision | `location_indoor/location_outdoor` теперь реально влияют на итоговые теплопотери труб и резервуаров. В проверенных первичных ТНП DOCX/XLSX этот множитель не найден, поэтому правило должно быть либо подтверждено отдельным источником, либо оставлено как явно задокументированная продуктовая поправка. |
