...
| Candidate Ant table fallback | Removed | 2026-06-01: candidate sizing table now has a single Glide path; `useElecCalcCandidateColumns.tsx`, candidate table-engine override and Ant branch removed; focused panel/unit + `ElecCalcPage` integration cover Glide props |
...
| Cable mark modal component | Done | 2026-06-01: manual cable mark dialog JSX moved to `ElecCalcCableMarkModal.tsx`; page keeps modal state, option builders and apply mutation flow; focused component unit + modal state/mutation units + `ElecCalcPage` integration |
