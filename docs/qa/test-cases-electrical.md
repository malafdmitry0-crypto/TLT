...
**Автоматизировано:** ✅ (frontend) `ElecCalcPage.test.tsx` — в активной строке колонки `Марка` доступны кнопки `Выбор` и `Подбор`
...
| 7 | Нажать `Подбор` в той же ячейке | Открывается полноэкранная модалка кандидатов; слева в таблице видны чекбокс выбора, действия и статус, далее все поля электрорасчёта; при открытии выполняется только чтение списка, авторасчёт не запускается |
...
| 10 | Нажать `Выбрать` у применимого кандидата | Backend пересчитывает основной электрорасчёт для текущего СО; ровно один кандидат объекта помечен `is_applied=true` |
