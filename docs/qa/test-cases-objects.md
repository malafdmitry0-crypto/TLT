...
| 2 | В toolbar нажать `Пересчитать все` или вызвать `POST /api/v1/calc/heat-loss/batch` с `project_id` | UI ставит `/calc/heat-loss/batch/jobs` без `object_ids`; sync API возвращает HTTP 200 |
...
| 4 | При активной строке нажать отдельную кнопку `Пересчитать все` | `object_ids` не передаются, пересчитывается весь проект |

---

## TC-OBJ-09B: Скрытие бокового блока параметров

**Автоматизировано:** ✅ (Playwright) `heat-calculation.spec.ts`

| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 1 | Установить расположение блока параметров справа или слева | Таблица и блок параметров делят рабочую область |
| 2 | Снять «Показать блок заполнения параметров» | Блок параметров и resize-разделитель полностью скрыты |
| 3 | Проверить рабочую область таблицы | Таблица занимает всю ширину layout, пустая правая/левая область не остаётся |
...
**UI-статус:** Excel-подобный режим в `HeatCalcPage` закрыт feature flag
`VITE_COMMERCIAL_FEATURES_ENABLED`; по умолчанию переключатель режима не
показывается. Сценарий ниже запускать только при включённом флаге.
...
**Normal mode Glide default:** обычный режим использует только Glide; старый fallback на AntD Table не поддерживается. Сортировка и фильтры таблицы закрыты тем же `VITE_COMMERCIAL_FEATURES_ENABLED`; при выключенном флаге header click и правая зона заголовка не показывают filter/sort controls и не меняют query. При включённом флаге Glide normal header click переключает сортировку, правая зона заголовка / правый клик по заголовку открывает фильтр; индикаторы сортировки и фильтра видны прямо в canvas-заголовке. В левом row-header отображаются чекбоксы мультивыбора; Ctrl/Cmd+клик переключает одну строку, Shift+клик выбирает диапазон от anchor-строки, а чекбокс заголовка выбирает/снимает видимые загруженные строки. Отдельная колонка `№` в normal Glide не показывается. Клик по строке открывает её в форме и подсвечивает активную строку синей полосой без сброса checkbox-selection; строки с ошибками остаются красными. Normal Glide не показывает page buttons: при скролле вниз он догружает следующую страницу через backend `page_info.next_cursor` и добавляет строки в текущий список.
