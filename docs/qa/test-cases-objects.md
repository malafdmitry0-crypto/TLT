...
| 4 | Выбрать и отредактировать ячейку после скролла | Editor открывается, draft строки обновляется |
| 5 | Открыть context menu после скролла | Меню Excel-режима открывается для дальней строки |
| 6 | Вставить блок 100×10 | Вставка завершается одним batch-сценарием без массового DOM |
| 7 | Проскроллить ниже последних данных колесом или scrollbar | В конце виден хвост пустых строк ввода; при дальнейшей прокрутке сетка продолжает расти как в Excel |
| 8 | Заполнить одну из последних пустых строк | Строка становится local draft row, ниже автоматически остаётся запас пустых строк для дальнейшего ввода |
| 9 | Сохранить | Сохраняются только заполненные local rows; untouched пустые строки не отправляются в backend |
| 10 | Повторить для 3000 строк | Поведение остаётся работоспособным; DOM-count не растёт до числа строк проекта |
