...
| 6 | В таблице выбрать строку с расчётной ошибкой вида `Температура горячей стороны слоя изоляции #N ... вне диапазона материала` | В форме параметров подсвечены красным `Материал изоляции`, `λ` и `Диапазон T` именно указанного слоя `#N`, без длинного текста ошибки внутри формы; верхний баннер объясняет, что это расчётная T на стороне трубы/продукта, и указывает эти поля для проверки |
...
| 5a | В Glide Excel нажать или протянуть по номерам строк слева | Выделяются строки целиком по ширине таблицы, форма параметров синхронизируется с активной строкой |
