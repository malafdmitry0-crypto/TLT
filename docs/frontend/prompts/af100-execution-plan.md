...
## 1. Порядок: AF100-06 идёт первым

Backlog сейчас указывает next = AF100-02. **Это нужно поменять на AF100-06.**

Причина — живая, а не косметическая: стандарт §7.4 уже объявил
`test:agent-dod:dual-safe` предпочтительным agent path, а эта команда падает
2/2 на чистом дереве. Каждый slice с `full_dod_required: true`, взятый сегодня,
упирается в §9 «полный gate красный → `blocked`» и останавливается на зелёном коде.
Пока AF100-06 не закрыт, вся очередь исполняется через sequential fallback.

Правильная последовательность зависимостей:

```text
AF100-06 (flake)  →  AF100-08 (профиль)  →  AF100-07 (канонизация)  →  AF100-09+ (скорость)
```

Канонизировать команду (07) до устранения флейка (06) — значит закрепить
красный CI. Оптимизировать (09+) до профиля (08) — запрещено самим планом §2.

Phase A slices (02–05) независимы друг от друга и от 06: их можно вести
параллельно с 06 разными запусками, они не трогают runtime.
