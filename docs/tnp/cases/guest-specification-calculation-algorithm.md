...
**Нормативная зависимость:** [действующий входной контракт Case1-r4](./electrical-input-contract-reconciliation.md)
...
Этот документ уточняет алгоритм спецификации, но не переопределяет действующий
[входной контракт Case1-r4](./electrical-input-contract-reconciliation.md).
...
| SPEC-DEC-02 | Внутри Case 1 расходятся §6.13 (стр. 43 и 45: заказная длина с запасом 10% передаётся в спецификацию) и §7.9 (стр. 61-62: в спецификацию передаётся фактическая сумма секций) | Кабель заказывать по `required_order_length_m`; инженерные аксессуары считать по фактической монтажной длине `actual_installed_length_m` |
...
### Зафиксированное расхождение по длине кабеля

Case 1 revision 4 содержит два несовместимых указания:

- §6.13, стр. 43 задаёт `base_order_length = installed_length * 1.1`, а
  стр. 45 прямо говорит, что установленная длина нужна для мощности, заказная
  длина - для закупки и спецификации;
- §7.9, стр. 61-62 рассчитывает `section_length_m * section_count` и прямо
  передаёт в спецификацию фактическую суммарную длину сформированных секций.

Текущий MVP намеренно следует первому варианту и решению `SPEC-DEC-02`:

```text
L_fact = section_length_m * section_count
required_order_length_m = ceil_to_0.001(L_fact * 1.10)
specification_cable_quantity = sum(required_order_length_m)
```

Запас 10% добавляется один раз в электротехническом расчёте. Спецификация
повторно на `1.10` не умножает. Ремонтные комплекты и крепёжные ленты используют
`actual_installed_length_m`, поэтому закупочный запас кабеля не увеличивает их
расход. Минимальная партия и кратность поставки из коммерческой ветки §6.13
остаются вне MVP согласно `DEC-02` электротехнического ТЗ.

Это согласование противоречия, а не дословное исполнение §7.9. Если владелец
бизнес-правила выберет фактическую длину, потребуется отдельное изменение
контракта ЭР -> спецификация и контрольных примеров; скрыто переключать формулу
нельзя.
...
immutable версию `builtin-specification/case1-demo-r4-v1` со `schema_version: 1`.
...
|---|---|---|---|---|---|
| Идентичность каталога | PDF не задаёт версию машиночитаемого каталога | `catalog_key: builtin-specification`, `version: case1-demo-r4-v1`, `schema_version: 1`, immutable | Нужна воспроизводимая и идемпотентная demo-поставка | Не является версией справочника владельца данных | Импортированным и утверждённым производственным каталогом с собственной версией |
