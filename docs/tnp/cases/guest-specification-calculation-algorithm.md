...
| SPEC-DEC-06 | Часть строк справочников в PDF не содержит кода или условий `Ex`/`Rгр` | Недостающие значения нельзя придумывать. Финальная спецификация блокируется для затронутой группы до подключения полного справочника |
...
1. Проверять только условия, значение которых не равно «не используется».
2. Булевы условия должны точно совпасть с фактом.
3. Если требуется `L_sec >= L_K2i_m`, сравнение включающее.
4. Если требуется `N_sec >= 3`, сравнение включающее.
5. При наличии заполненных условий `Ex` и `R_gr` проверить их по правилам полного справочника.
...
Матрица на странице 76 не содержит значений `condition_Ex` и `condition_R_gr`, хотя эти поля объявлены на странице 75. До регистрации авторитетных значений система не должна придумывать их или считать текстовое описание формул.
...
| Не зарегистрированы `Ex`/`R_gr` условия коробок | Не угадывать матрицу; блокировать финализацию затронутой коробочной части |
...
1. Полная матрица `condition_Ex` и `condition_R_gr` для 12 соединительных коробок.
