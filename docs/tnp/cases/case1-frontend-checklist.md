...
- [ ] `Iдоп`: настройка проекта + переопределение на объекте; при отсутствии — блокирующее
      состояние «Задать допустимый стартовый ток» (`SECTION_CURRENT_LIMIT_REQUIRED`), секции не
      показываются как успешные.
...
- [ ] Сохранение проекта в файл (кейс 4.2, 5.11–5.12): файл должен содержать объекты, исходные
      данные, результаты расчётов, порядок строк, ЭР и спецификацию; сейчас «Скачать/Загрузить» —
      CSV (по-видимому, только объекты). Проверить и расширить до полного файла проекта.
