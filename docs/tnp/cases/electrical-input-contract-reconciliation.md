...
9. Проверка нижнего предела температуры среды (`T_env < T_min` из кейса, стр. 42-43) нормализована
   ТЗ не как фильтр кандидатов до выбора модели, а как fail-closed поиск строки секционирования по
   cold-start после выбора (см. 6.7). Это документированное отступление от порядка шагов кейса, а
...
### 6.7 Температура окружающей среды (`T_env` / `T_min`)

Кейс, стр. 42-43: `если T_env < T_min - марка не подходит`, и в auto-режиме (§6, шаг 5) это фильтр
кандидатов **до** выбора минимального варианта. Нормализованное ТЗ сознательно не включает `T_env`
в пригодность 9.5: нижний предел реализуется на этапе секционирования - `lookup_section_row`
ищет строку по cold-start как `exact/nearest-colder, never warmer`
([sections.py](../../../backend/app/formulas/electrical/sections.py)); объект холоднее самой
холодной строки каталога не получает строку и расчёт завершается fail-closed, без подстановки
более тёплой температуры.

Зафиксированные следствия нормализации:

- проверка выполняется после выбора модели, а не фильтром кандидатов. В поставляемом
  `section_catalog.json` минимальный cold-start одинаков у всех моделей всех серий (`-40`),
  поэтому перебор следующего кандидата после провала секционирования в MVP не требуется -
  провалились бы все кандидаты серии одинаково;
- ошибка обязана называть причину (нет строки секционирования для данного cold-start) с полем и
  температурой, а не выглядеть generic-сбоем каталога;
...
- Объект с cold-start холоднее самой холодной строки секционирования получает типизированную
