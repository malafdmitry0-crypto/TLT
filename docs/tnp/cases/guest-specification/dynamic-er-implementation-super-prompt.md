...
В отдельной Git-ветке заменить фиксированную модель вариантов расчёта
...
2. Не удаляй, не перемещай и не откатывай чужие dirty/untracked-файлы.
3. Реализацию вести в отдельной ветке с рекомендуемым именем:
   `feature/tnp-dynamic-electrical-variants`.
4. Если исходная рабочая копия dirty, предпочти отдельный Git worktree для
   новой ветки. Не используй `git reset --hard`, `git clean` или checkout чужих
   файлов.
5. Не делай commit, push, merge или PR без отдельного разрешения пользователя.
6. До завершения Phase 0 production-код не менять.
...
формулу/каталог и независимый oracle. Не возвращай PDL-ER-01…25 в статус
...
PDL-ER-18…25 — 18.07.2026 также как варианты А. Открытыми остаются guest
persistence/TTL, лимит 500 объектов и фактическое предоставление официального
числового источника PDL-ER-15/18. Они не должны молча расширять scope.
...
- frontend должен перейти на UUID в том же feature branch;
- задать дату/условие удаления compatibility layer;
- не поддерживать два независимо изменяемых источника истины.

Если внешних клиентов нет, выполнить атомарный hard cut внутри ветки и удалить
...
- остановиться до фактического предоставления официального числового каталога/
  ...
  округления по PDL-ER-15/18; не вводить defaults;
...
- PDL-ER-01…25 реализованы без скрытых альтернативных semantics;
...
- обнаружена новая необходимая семантика вне PDL-ER-01…25 и действующих
...
3. подтверждение применения PDL-ER-01…25 и список только новых blockers;
