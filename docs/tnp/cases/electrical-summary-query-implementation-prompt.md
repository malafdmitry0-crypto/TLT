...
Нормативный источник — `1_Кейс`, в частности §§6.8, 6.14 и 6.16. Действующие разрешения неоднозначностей фиксирует `electrical-input-contract-reconciliation.md`. Backend остаётся авторитетом для формул, каталогов, округлений, read scope и summary; frontend отображает серверный контракт и управляет query cache.
