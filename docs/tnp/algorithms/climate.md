...
- В приложении строка климатического справочника выбирается по стабильному
  `climate_key = region|||city`; если ключа нет, backend использует точную пару
  `climate_region + climate_city`. Fallback по одному названию города разрешён
  только для однозначных городов, чтобы не выбирать неверную строку при
  дубликатах названий и сохранить совместимость со старыми объектами.
...
- Нужен regression case для города-дубликата: `Октябрьское` в ХМАО и
  `Октябрьское` в Челябинской области имеют разные `T0/T1`, поэтому выбор
  должен идти по ключу/региону.
