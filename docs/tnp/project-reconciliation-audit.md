...
Самое важное оставшееся расхождение находится в полном VSDX-автоподборе
резистивного кабеля. Базовая паспортная часть `R/P/I/65А`, climate policy,
`Kn_max(D)`, T3 fallback и full-version policy ниток для ТТН/ТТВ/ТТХ уже
перенесены в backend/QA-agent.
...
|---|---|---|---|---|
| `ALG-RES-01` | Fixed/Partial | Резистивный кабель | VSDX-алгоритм работает с `Q(i,1)` как номинальным сопротивлением кабеля. Backend теперь для паспортных строк считает `R = resistance_ohm_km / 1000 * length`, `P = U^2 / R`, `I = P/U` и применяет лимит `65 А`; legacy cross-section остается совместимым fallback для каталогов без сопротивления. | Для полной версии осталось формализовать автоподбор `U/N/M`, `p2/p3`, `L1/L2`. |
| `ALG-RES-02` | High | Резистивный кабель | VSDX содержит итеративный подбор: сортировка линейки, расчет `p2`, ограничение `p3` по максимальной мощности/току `65 А`, переходы `U=220/380`, `N=2/3`, увеличение `M`. Backend сейчас принимает схему и число ниток как вход, но не перебирает полную стратегию. | Реализовать автоподбор схемы по VSDX; ручной ввод схемы может остаться как override, но не заменяет алгоритм полной версии. |
...
| `REF-INS-01` | Medium | Теплоизоляция | ТНП-справочник содержит формулы `lambda(tm)`, но backend loader возвращает фиксированное `conductivity`; это не соответствует полной версии для материалов с температурной зависимостью. | Реализовать temperature-dependent insulation conductivity для материалов с формулой. |
...
1. Реализовать полный deterministic oracle для VSDX `resistive-selection`:
   паспортные длины секций, `p2/p3`, `U/N/M`, `L1/L2`.
2. Переподключить auto-resistive selection к полной VSDX-стратегии; текущий
   ручной выбор схемы может остаться как override/diagnostic path.
3. Реализовать `lambda(tm)` для теплоизоляции там, где в справочнике есть
   формула.
