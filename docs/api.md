...
Для ТЛТ (`self_regulating`) `process_temperature` обязателен: endpoint заполняет
его из параметров объекта, если поле не передано явно, и возвращает ошибку,
если температуры продукта нет ни в payload, ни в объекте. Это нужно для
проверки `max_temperature` выбранного кабеля.
