# Исторический аудит соответствия бизнес-логики ТЗ

> [!WARNING]
> Срез от 2026-05-20. Старые test counts, `СО1…СО4`, UI-макеты и проценты
> готовности ниже сохранены как evidence и не являются текущей матрицей.
> Текущий статус — `codex-docs/requirements-map.md`, актуальные counts —
> generated-блок `README.md`.

Таблица сверки ТЗ (Приложение № 1 + Приложения 1–4) с фактической реализацией
в коде и тестами. Дата актуализации: **2026-05-20**.
...
**Статус тестов на момент актуализации:** Backend `1191` (882 unit + 309 integration) ✅ ·
Frontend vitest `372` ✅ · E2E Playwright `64` ✅. Счётчики в `README.md` (блок
`AUTO:test-counts`) обновляются через `scripts/sync-docs.py`.
...
| 4.2.1 Формулы строго из ТНП для базового уровня | ✅ | Основные формулы трубы/резервуара покрыты; изоляция считается по `lambda(tm)` и конкретному материалу/плотности |
| 4.2.1 Климат / теплопроводность / параметры — встроенные справочники | ✅ | `reference_data/{climate,insulation,pipe_materials,soil_conductivity,cables_tlt,resistive_cables,accessories}.json` + `loader.py`; изоляция использует температурозависимую λ и сидируется в `insulation_materials` для runtime `/references/insulation` |
...
| 4.3.1 Все формулы «Полной версии» из ТНП | ❌ | Business contract закрывает теплопотери (в т.ч. `lambda(tm)`), ТЛТ, ТТН/ТТВ/ТТХ, ТТ Р1/ТТ Р3; остаются MI/skin и pump/platform/other — см. [open-business-decisions.md](analysis/open-business-decisions.md) |
| 4.3.2 Все типы кабеля (саморег, одножил, трёхжил, минер. изол., скин) | ⚠️ | Расчётно: ТЛТ, ТТН/ТТВ/ТТХ, ТТ Р1 (`single_core`), ТТ Р3 (`three_core`); **mineral** и **skin** — только каталог, расчёт ждёт методику (CAB-MIN, CAB-SKIN) |
...
| 4.3.6 Управление ключами шифрования (зависит от выбора в SRS) | ✅ | N/A при варианте А; UI ключей нужен только для варианта С (SEC-C — Open) |
...
| Расшифровка кодов ошибок | ⚠️ | Черновик `docs/error-codes.md`; полный каталог — Open (DOC-ERR) |
...
| Раздел ТЗ | Готовность | Вес в итоге | Комментарий |
|---|---:|---:|---|
| 4.1 Общие требования | **≈97%** | 12% | DnD строк в UI; Excel-like детали — Q-B |
| 4.2 Базовый уровень | **≈98%** | 13% | Гостевой контур; остаток — production-данные справочников |
| 4.3 Полная версия | **≈82%** | 42% | +ТТ Р1/Р3/ТТН в коде; −mineral/skin, pump/platform/other |
| 5 Безопасность | **≈68%** | 18% | Auth/RBAC ✅; SEC-A подпись и SEC-02 — Open |
| 6 Документация | **≈92%** | 10% | Черновик `error-codes.md`; полный каталог — Open |
| Приложение 4 (эскизы) | **100%** | 5% | Рис. 1–3 |

**Взвешенная готовность по полному ТЗ: ≈85%** (код в закрытом контуре; подписание
без оговорок блокируют [open-business-decisions.md](analysis/open-business-decisions.md)).

| Критерий (вне весов ТЗ) | Оценка |
|---|---:|
| Базовый уровень 4.2 (роль «Пользователь») | **≈97%** |
| Формулы в business contract (реализованный контур) | **≈93%** |
| Автоматизация ключевых сценариев ПМИ (E2E) | **≈72%** |
