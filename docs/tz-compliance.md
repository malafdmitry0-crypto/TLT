...
в коде и тестами. Дата актуализации: **2026-05-17**.
...
**Статус тестов на момент актуализации:** Backend `977` ✅ · Frontend vitest `310` ✅ ·
E2E Playwright `58` ✅. Числа синхронизируются через `scripts/sync-docs.py`.
...
| 4.3.1 Все формулы «Полной версии» из ТНП | ❌ | Текущий business contract закрывает ТЛТ, ТТН/ТТВ/ТТХ, ТТ Р1/ТТ Р3; остаются `lambda(tm)`, MI/skin, pump/platform/other и часть финальных бизнес-решений |
...
| 4.2 Базовый уровень | **≈98%**: пользовательский контур работает, температурозависимая λ изоляции реализована; остатки связаны с наполнением production-справочников/закупочных данных |
