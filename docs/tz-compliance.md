...
| 4.2.1 Формулы строго из ТНП для базового уровня | ⚠️ | Основные формулы трубы/резервуара покрыты; открытый разрыв полной версии — `tlt_insulation_lambda_tm`: backend пока берёт фиксированную `conductivity`, а не `lambda(tm)` |
| 4.2.1 Климат / теплопроводность / параметры — встроенные справочники | ⚠️ | `reference_data/{climate,insulation,pipe_materials,soil_conductivity,cables_tlt,resistive_cables,accessories}.json` + `loader.py`; для изоляции нужна температурная зависимость |
...
| 4.2 Базовый уровень | **≈95%**: пользовательский контур работает, но для полного ТНП-контракта нужна температурозависимая λ изоляции |
