...
Сотрудник проверяет расчёты, сделанные гостем-подрядчиком. Диаграмма показывает ветвления при обнаружении ошибок и финальный экспорт для подписи главным инженером.

```mermaid
flowchart TD
    Start([Уведомление: подрядчик\nсдал расчёты]) --> Login[Войти по логину/паролю\nПолучить JWT]

    Login --> OpenProjects[Открыть список всех проектов\nGET /projects?all=true]

    OpenProjects --> FindProject[Найти проект подрядчика\nпо названию / дате]
