...
### 7.1 Дополнительный замер — `isolate: false`

Прямой способ не платить за `antd` на каждом файле — не пересоздавать модульный
реестр, чтобы библиотека грузилась **раз на воркер**. Измерено на unit-проекте:

| | `isolate: true` (сейчас) | `isolate: false` |
|---|---:|---:|
| import (collect) | 89.7 s | **12.7 s (−86 %)** |
| setup | 19.5 s | 2.7 s |
| **wall** | 167.9 s | **84.1 s (−50 %)** |
| результат | 1202 passed | **92 failed** в 27 файлах |

Механизм работает и с запасом закрывает бюджет p50 ≤120 s. Блокирует его не
скорость, а 27 файлов, которым нужен чистый модульный реестр:

| Причина падения | Тестов |
|---|---:|
| не сброшенное состояние моков/шпионов | 37 |
| прочее | 25 |
| timeout / `waitFor` | 11 |
| дубли элементов в DOM | 5 |
| протёкший синглтон стора | 1 |

**Конфигурационного обхода нет.** `clearMocks` + `mockReset` + `restoreMocks` +
`unstubEnvs` + `unstubGlobals` дали 92 → 86 падений и при этом **замедлили**
прогон до 156.9 s (tests 141 → 228 s): принудительный сброс сам загоняет тесты
в таймауты. Починка — пофайловая, по 27 файлам.

### 7.2 Развилка для AF100-09b

Два измеренных пути, оба требуют отдельного слайса:

| Путь | Выигрыш | Цена |
|---|---|---|
| `deps.optimizer` | −78 % collect | найти причину дублирования инстансов antd (не `antd/es/*`, см. §2) |
| `isolate: false` | −86 % collect, −50 % wall | починить 27 протекающих файлов **и** ввести guard: без изоляции связанность между тестовыми файлами перестаёт падать сама |

Второй путь измерен полнее и даёт больше, но **навсегда снимает** свойство
безопасности: сейчас перекрёстная зависимость между тестовыми файлами
невозможна физически. Возможен и гибрид — `isolate: false` для 260 здоровых
файлов при сохранении изоляции для 27 (Vitest допускает per-project настройку).
Выбор пути — предмет 09b, не этого слайса.
