...
export default function CableAlgorithmPanel({
  ...
  const processTemperature = Form.useWatch('process_temperature', form);
  ...
  const processDisplay = processTemperature === null || processTemperature === undefined || processTemperature === ''
    ? '—'
    : String(processTemperature);

  return (
    <div
      className="object-wizard-cable-panel"
    ...
    >
      ...
      <div className="cable-algorithm-grid" data-layout="cable-algorithm">
        {/* 1. process_temperature — mirror left heat form (shared source of truth) */}
        <div className="cable-algorithm-field cable-algorithm-field--mirror">
          <div className="ant-form-item-label">
            <label title="Температура жидкости внутри объекта">
              <FieldLabel text="T жидкости (из теплорасчёта)" />
            </label>
          </div>
          <div
            className="cable-algorithm-mirror-value"
            data-testid="cable-process-temperature-mirror"
            title="Синхронизировано с полем «Требуемая температура объекта»"
          >
            {processDisplay}
            <span className="cable-algorithm-mirror-unit">°C</span>
          </div>
        </div>
        ...
        >
          {withHelp(
            <TltNumberField
              ...
              className="cable-algorithm-number"
            />,
            'Коэффициент навива (1–1,5). Используется алгоритмом выбора кабеля (ТНП).',
          )}
        </Form.Item>
        ...
        <Form.Item
          ...
          rules={heatCalcFormFieldRules(form, objectType, 'temperature_group')}
        >
          {withHelp(
            <TltSelect
        ...
        {/* 9. connection_type / схема соединения */}
        <Form.Item
