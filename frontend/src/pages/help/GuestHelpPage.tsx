    ...
    <div className="help-page">
      <div className="help-page-inner help-page-inner--guest">
        ...
        <TltCard>
          ...
          <Paragraph>
            <ul>
              <li><Text strong>Наружный диаметр</Text> — в миллиметрах (например, 57, 89, 108).</li>
              ...
              <li><Text strong>Минимальная температура окружающей среды</Text> — расчётная минимальная температура наружного воздуха, °C; участвует в расчёте теплопотерь</li>
              <li><Text strong>Максимальная температура окружающей среды</Text> — необязательный справочный параметр, °C; если он указан, то должен быть не ниже минимальной температуры. В формуле теплопотерь максимум не используется</li>
          ...
          <Paragraph>
            <ul>
              ...
              <li><Text strong>Температуры и изоляция</Text> — минимальная температура используется в расчёте, а необязательная максимальная хранится для справки и должна быть не ниже минимальной</li>
            </ul>
          </Paragraph>
          <Paragraph>
            Для подземного трубопровода обе границы температуры воздуха не применяются и
            скрыты: расчёт использует температуру грунта. У подземной ёмкости минимальная и
            максимальная температуры окружающей среды сохраняются.
          </Paragraph>
          ...
          <Paragraph>
            <ul>
              ...
              <li>
                <Text strong>Этап 2 — Электрорасчёт.</Text> Объект <strong>валидный физически</strong>, но не укладывается в выбранный тип кабеля. Встроенные формулы есть для ТЛТ, ТТН/ТТВ/ТТХ, ТТ Р1 и ТТ Р3; новые типы систем подключаются вместе со своей формулой и каталогом.
